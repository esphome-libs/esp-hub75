...
constexpr bool is_four_scan_wiring(Hub75ScanWiring wiring) {
  return wiring == Hub75ScanWiring::FOUR_SCAN_16PX_HIGH || wiring == Hub75ScanWiring::FOUR_SCAN_32PX_HIGH ||
         wiring == Hub75ScanWiring::FOUR_SCAN_64PX_HIGH;
}
 ...
 *
 * For FOUR_SCAN_16PX_HIGH (16px high, 1/4 scan): segments of panel_width
 * For FOUR_SCAN_32PX_HIGH (32px high, 1/8 scan): 16-pixel segments
 * For FOUR_SCAN_64PX_HIGH (64px high, 1/8 scan): segments of panel_width
 *
...
consteval bool test_four_scan_no_overflow() {
  ...
  constexpr Coords out16 = ScanPatternRemap::remap(input, Hub75ScanWiring::FOUR_SCAN_16PX_HIGH, panel_width, 16);
  constexpr Coords out32 = ScanPatternRemap::remap(input, Hub75ScanWiring::FOUR_SCAN_32PX_HIGH, panel_width, 32);
  constexpr Coords out64 = ScanPatternRemap::remap(input, Hub75ScanWiring::FOUR_SCAN_64PX_HIGH, panel_width, 64);
...
consteval bool test_four_scan_dma_dimensions() {
  constexpr uint16_t panel_width = 64;
  constexpr uint16_t panel_height = 32;

  // For 64x32 1/8 scan panel:
  // dma_width = panel_width * 2 = 128
  constexpr uint16_t dma_width = get_effective_dma_width(Hub75ScanWiring::FOUR_SCAN_32PX_HIGH, panel_width, 1, 1);
  // num_rows = panel_height / 4 = 8
  constexpr uint16_t num_rows = get_effective_num_rows(Hub75ScanWiring::FOUR_SCAN_32PX_HIGH, panel_height);

  return dma_width == 128 && num_rows == 8;
}

// Validate coordinate remapping for 64x32 1/8 scan panel
consteval bool test_four_scan_32px_remap() {
  constexpr uint16_t panel_width = 64;
  constexpr uint16_t panel_height = 32;
...
static_assert(test_four_scan_32px_segment_size(), "FOUR_SCAN_32PX_HIGH segment size should be 16 for 64x32 panel");
static_assert(test_four_scan_dma_dimensions(), "Four-scan DMA dimensions incorrect");
static_assert(test_four_scan_32px_remap(), "FOUR_SCAN_32PX_HIGH coordinate remapping incorrect");
